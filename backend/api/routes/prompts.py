...
async def buy_prompt(prompt_id: str, user=Depends(get_current_user)):
    """Purchase a prompt"""
...
async def rate_prompt(
    prompt_id: str, rating: int, review: Optional[str] = None, user=Depends(get_current_user)
):
    ...
    return {"status": "rated", "prompt_id": prompt_id, "rating": rating, "review": review}
