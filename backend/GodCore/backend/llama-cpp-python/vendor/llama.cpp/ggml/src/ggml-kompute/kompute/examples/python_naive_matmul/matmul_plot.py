        ...
        for x in range(tensor.shape[0]):
            if coord_highlight and x == coord_highlight[1] and y == coord_highlight[0]:
                cv2.putText(
                    image,
                    str(int(tensor[y, x])),
                    (x * font_size, int((y + 0.8) * font_size)),
...
def main():
    ...
    for workgroup_x in range(workgroup[0]):
        for workgroup_y in range(workgroup[1]):
            for invocation_x in range(
                workgroup_x * local_size[0], (workgroup_x + 1) * local_size[0]
            ):
                for invocation_y in range(
                    workgroup_y * local_size[1], (workgroup_y + 1) * local_size[1]
                ):
                    row = invocation_x
                    col = invocation_y
